-- FUNCTION: public.view_spotcash_orissuance(character varying, character varying, character varying, date, date, integer, integer, boolean)

-- DROP FUNCTION IF EXISTS public.view_spotcash_orissuance(character varying, character varying, character varying, date, date, integer, integer, boolean);

CREATE OR REPLACE FUNCTION public.view_spotcash_orissuance(
...
BEGIN
	...
	(
		...
		from
		(
			select false as tag, (case when p_issued then coalesce(a.si_no,a.or_no) else null end)::varchar as or_no, 
					...
					case
						when p_issued = true
							then exists(select * from rf_payments y where y.pay_rec_id::int = x.pay_rec_id::int and (y.remarks ~* 'Late LTS/BOI' or y.remarks ~* 'Late OR Issuance for Good Check'))
								and (x.or_doc_id is not null or trim(x.or_doc_id) != '' 
								...
								/* MODIFIED BY JARI CRUZ ASOF JAN 16 2023
								and (x.or_doc_id is null or trim(x.or_doc_id) = '')
								*/
								and (coalesce(x.si_doc_id,x.or_doc_id) is null or trim(x.or_doc_id) = '')
					end
				)
				and
				(
					...
					case
						...
						(
							...
							(
								...
								where z.type_id = y.buyertype and z.type_group_id IN ('02', '04', '03')
							)
						...
						(
							...
							(
								select * 
								from mf_buyer_type z
								where z.type_id = y.buyertype and z.type_group_id IN ('02', '04','03') -- added 03 by jari cruz asof dec 14 2022 ndi kasi nasasama mga cash and deferred
							)
							and y.entity_id = x.entity_id and y.projcode = x.proj_id 
							and TRIM(y.pbl_id) = TRIM(x.pbl_id) and y.seq_no = x.seq_no and y.status_id = 'A'
						)
			...
			WHEN a.entity_id in ('1185209653') THEN TRUE -- added by jari cruz asof august 24, 2022, reason special case; Client: HERNANDEZ, KAREN MAE L. 
			...
			--and a.or_no not in (select receipt_id from rf_payments where status_id != 'I' and coalesce(receipt_id,'') != '')
				AND CASE WHEN a.server_id IS NOT NULL THEN NULLIF(trim(a.or_no), '') IS NOT /*NOT is added by jari cruz sept 1 2022 reason itsreal client*/ NULL AND exists (select * from rf_buyer_status x where x.entity_id = a.entity_id and x.proj_id = a.proj_id and x.pbl_id = a.pbl_id and x.seq_no::int = a.seq_no::int and TRIM(x.status_id) = 'A' and TRIM(x.byrstatus_id) = '01') ELSE
			...
			select false as tag, (case when p_issued then coalesce(a.si_no, a.or_no) else null end)::varchar as or_no, 
			(case when p_issued then coalesce(a.si_date, a.or_date) else null end)::date as or_date, f.entity_name as name, e.description as unit, 
			...
			(
				...
				(
					case
						when p_issued = true
							then exists(select * from rf_payments y where y.pay_rec_id::int = x.pay_rec_id::int and (y.remarks ~* 'Late LTS/BOI' or y.remarks ~* 'Late OR Issuance for Good Check'))
								and (COALESCE(x.si_doc_id, x.or_doc_id) is not null or trim(x.or_doc_id) != '')
						else NOT exists(select * from rf_payments y where y.pay_rec_id::int = x.pay_rec_id::int and (y.remarks ~* 'Late LTS/BOI' or y.remarks ~* 'Late OR Issuance for Good Check'))
								/* MODIFIED BY JARI CRUZ ASOF JAN 16 2023
								and (x.or_doc_id is null or trim(x.or_doc_id) = '')
								*/
					...
					case
						when p_issued = true
							...
							(
								case
									when p_datefilterindex = 0
										then COALESCE(x.si_date::date, x.or_date::date)
									when p_datefilterindex = 1
										then (select y.date_issued::date from rf_orlist y where y.pay_rec_id::int = x.pay_rec_id::int AND status_id = 'A')
								end
							) between p_datefrom and p_dateto
			...
			where CASE 
			WHEN a.server_id IS NOT NULL THEN TRUE
			WHEN a.entity_id in ('1185209653') THEN TRUE -- added by jari cruz asof NOV 7, 2022, reason special case; Client: HERNANDEZ, KAREN MAE L. 
			/*
			ORIGINAL CODE MODIFIED BY JARI CRUZ AS OF DEC 21 2022
			ELSE a.pay_part_id in (SELECT pay_part_id FROM mf_pay_particular where receipt_to_issue IN ('01','307') and status_id != 'I') END 
			*/
			ELSE a.pay_part_id in (SELECT pay_part_id FROM mf_pay_particular where receipt_to_issue IN ('01','307') and status_id != 'I') END 
			and (case when a.pymnt_type = 'B' then a.check_stat_id = '01' else a.pay_rec_id is not null end) 
			...
			*/
			...
			(
				...
				x.check_date, a.check_stat_id, x.bounce_reason_id, x.or_no, x.or_date, x.ar_no, x.brstn, x.request_no, x.applied_amt, x.cancelled, x.remarks, x.branch_id, x.post_date, 
				x.client_seqno, x.or_doc_id, x.pr_doc_id, x.status_id, x.wdraw_stat, x.date_wdrawn, x.wdraw_no, x.wdraw_reason, x.repl_wdraw_by, x.date_remitted, x.remit_batch, x.reversed, 
	...
	LOOP
		...
		if c_pay_rec_id in (778321, 778322) THEN
			c_jv_no := '24060024';
		END IF;

		c_jv_no		:= coalescE(c_jv_no, ''); 
		
		IF v_or.server_id IS NOT NULL THEN
			c_percentage    := (SELECT ROUND(c_percent_paid, 2) FROM view_card_ledger_with_moratorium_v2(v_or.entity_id, v_or.proj_id, v_or.pbl_id, v_or.seq_no, false) order by c_percent_paid desc limit 1);
		ELSE
		...
		*/
		...
		if p_listfilterindex = 1 then
			RAISE INFO '**********';
			RAISE INFO 'NAME: %',v_or.name;
			RAISE INFO 'GROUP ID: %',get_group_id(v_or.buyertype);
			RAISE INFO 'PART ID: %',trim(v_or.pay_part_id);
			RAISE INFO '**********';
		/*
...
ALTER FUNCTION public.view_spotcash_orissuance(character varying, character varying, character varying, date, date, integer, integer, boolean)
    OWNER TO postgres;

GRANT EXECUTE ON FUNCTION public.view_spotcash_orissuance(character varying, character varying, character varying, date, date, integer, integer, boolean) TO PUBLIC;

GRANT EXECUTE ON FUNCTION public.view_spotcash_orissuance(character varying, character varying, character varying, date, date, integer, integer, boolean) TO employee;

GRANT EXECUTE ON FUNCTION public.view_spotcash_orissuance(character varying, character varying, character varying, date, date, integer, integer, boolean) TO postgres;
